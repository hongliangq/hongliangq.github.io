...
draft: false
title: 'How to Understand Probability Path'
# categories: ["Flow Matching"]
# tags: ["Math", "Diffusion"]
---

## Definition

In flow matching, a probability path is the continuous evolution of a probability distribution $p_t$ that smoothly transports the source distribution $p_0$ into the target distribution $p_1$ over time $t \in [0,1]$.

## Push-Forward

We start with a random variable $X \sim p_X$ and we apply a transformation $Y = \psi(X)$ where the function $\psi$ is a diffeomorphic map.
...
Sure, one could sample from $X$ and apply $\psi$. But often in theory, statistics, and ML, we need an **explicit formula for the density** $p_Y$. 
...
$$
p_Y(y) = p_X \bigl(\psi^{-1}(y) \bigr) \cdot  \big|\det \partial_y \psi^{-1}(y)\big|.
$$
This tells us how the density ***reshapes*** when the space itself is distorted by $\psi$.
...
Finally, here comes the question: ***how is push-forward explained as probability path?***
